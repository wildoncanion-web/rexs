...
export function DashboardHeader() {
  ...
  return <header className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur-xl">
    <div className="mx-auto flex h-16 max-w-[1500px] items-center gap-5 px-4 lg:px-8">
      <Link href="/dashboard" className="flex shrink-0 items-center gap-2.5"><span className="flex size-9 items-center justify-center rounded-lg bg-primary font-mono text-lg font-bold text-primary-foreground">IH</span><span className="hidden text-sm font-semibold tracking-wide text-foreground sm:inline">INVESTMENT HOLDINGS, LLC</span></Link>
      <nav className="hidden items-center gap-5 lg:flex"><Link href="/dashboard" className="text-sm font-medium text-foreground">Portfolio</Link><Link href="/dashboard/deposit" className="text-sm text-muted-foreground hover:text-foreground">Deposit</Link><Link href="/dashboard/withdraw" className="text-sm text-muted-foreground hover:text-foreground">Withdraw</Link><Link href="/dashboard/transactions" className="text-sm text-muted-foreground hover:text-foreground">Markets</Link><Link href="/faq" className="text-sm text-muted-foreground hover:text-foreground">Research</Link></nav>
    ...
    {mobileMenuOpen && <div className="border-t border-border px-4 py-4 lg:hidden"><nav className="flex flex-col gap-4"><Link href="/dashboard" onClick={() => setMobileMenuOpen(false)} className="text-sm font-medium">Portfolio</Link><Link href="/dashboard/deposit" onClick={() => setMobileMenuOpen(false)} className="text-sm text-muted-foreground">Deposit</Link><Link href="/dashboard/withdraw" onClick={() => setMobileMenuOpen(false)} className="text-sm text-muted-foreground">Withdraw</Link><Link href="/dashboard/transactions" onClick={() => setMobileMenuOpen(false)} className="text-sm text-muted-foreground">Markets</Link><Link href="/faq" onClick={() => setMobileMenuOpen(false)} className="text-sm text-muted-foreground">Research</Link></nav></div>}
  </header>
}
