        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,65fr)_minmax(300px,35fr)]">
          <section className="flex min-w-0 flex-col gap-6">
            <Card className="border-border bg-card shadow-none">
              <CardContent className="p-6 md:p-8">
                ...
                <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
                  <p className="text-4xl font-semibold tracking-tight text-card-foreground md:text-5xl">$124,500.00</p>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <Card className="border-border bg-card shadow-none"><CardHeader className="flex-row items-center justify-between space-y-0 pb-3"><CardTitle className="text-base font-semibold tracking-tight">Asset Allocation</CardTitle><Button variant="ghost" size="sm" className="text-xs text-muted-foreground">View details</Button></CardHeader><CardContent><div className="flex items-center gap-6"><div className="relative size-32 shrink-0"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={allocation} dataKey="value" innerRadius={43} outerRadius={60} paddingAngle={3} strokeWidth={0}>{allocation.map((entry) => <Cell key={entry.name} fill={entry.color} />)}</Pie></PieChart></ResponsiveContainer><div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-lg font-semibold">100%</span><span className="text-[10px] text-muted-foreground">allocated</span></div></div><div className="flex flex-1 flex-col gap-3">{allocation.map((item) => <div key={item.name} className="flex items-center justify-between text-sm"><span className="flex items-center gap-2 text-muted-foreground"><span className="size-2 rounded-full" style={{ backgroundColor: item.color }} />{item.name}</span><span className="min-w-12 text-right font-mono text-sm font-semibold tabular-nums text-card-foreground">{item.value}%</span></div>)}</div></div></CardContent></Card>
              <Card className="border-border bg-card shadow-none"><CardHeader className="flex-row items-center justify-between space-y-0 pb-3"><CardTitle className="text-base font-semibold tracking-tight">Recent Transactions</CardTitle><Link href="/dashboard/transactions" className="text-xs text-primary hover:underline">View all</Link></CardHeader><CardContent className="flex flex-col gap-4">{transactions.map((item) => <div key={item.title} className="flex items-center gap-3"><div className="flex size-9 shrink-0 items-center justify-center rounded-full bg-secondary text-xs font-medium text-muted-foreground">{item.date.slice(3)}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-card-foreground">{item.title}</p><p className="text-xs text-muted-foreground">{item.detail}</p></div><span className={item.positive ? "text-sm font-medium text-primary" : "text-sm font-medium text-destructive"}>{item.amount}</span></div>)}</CardContent></Card>
            </div>
          </section>

          <aside className="flex min-w-0 flex-col gap-6 lg:sticky lg:top-24 lg:self-start">
            <Card className="border-border bg-card shadow-none"><CardHeader className="flex-row items-center justify-between space-y-0"><CardTitle className="text-base">Watchlist</CardTitle><Button variant="ghost" size="icon" aria-label="Search watchlist"><Search /></Button></CardHeader><CardContent className="flex flex-col gap-1">{watchlist.map((item) => <div key={item.symbol} className="flex items-center gap-3 rounded-lg px-2 py-3 transition-colors hover:bg-secondary"><div className="flex size-9 items-center justify-center rounded-md bg-secondary font-mono text-xs font-semibold text-card-foreground">{item.symbol.slice(0, 1)}</div><div className="min-w-0 flex-1"><p className="font-mono text-sm font-semibold text-card-foreground">{item.symbol}</p><p className="truncate text-xs text-muted-foreground">{item.name}</p></div><MiniSparkline points={item.points} positive={item.positive} /><div className="text-right"><p className="text-sm font-medium text-card-foreground">{item.price}</p><Badge variant={item.positive ? "default" : "destructive"} className="mt-1 px-1.5 py-0 text-[10px]">{item.change}</Badge></div></div>)}</CardContent></Card>
            <Card className="border-border bg-card shadow-none"><CardHeader><CardTitle className="text-base">Quick Trade</CardTitle><p className="text-sm text-muted-foreground">Place an order in your portfolio</p></CardHeader><CardContent className="flex flex-col gap-4"><ToggleGroup type="single" value={tradeType} onValueChange={(value) => value && setTradeType(value)} className="grid grid-cols-2 rounded-md bg-secondary p-1"><ToggleGroupItem value="buy" className="h-9 rounded-sm text-sm data-[state=on]:bg-card data-[state=on]:text-primary">Buy</ToggleGroupItem><ToggleGroupItem value="sell" className="h-9 rounded-sm text-sm data-[state=on]:bg-card data-[state=on]:text-destructive">Sell</ToggleGroupItem></ToggleGroup><label className="flex flex-col gap-2 text-sm font-medium text-card-foreground">Ticker Symbol<Input placeholder="e.g. AAPL" className="h-11 border-border bg-secondary/50 font-mono text-card-foreground uppercase placeholder:text-muted-foreground" /></label><label className="flex flex-col gap-2 text-sm font-medium text-card-foreground">Amount ($)<Input type="number" placeholder="0.00" className="h-11 border-border bg-secondary/50 font-mono text-card-foreground placeholder:text-muted-foreground" /></label><Button className="h-11 w-full bg-primary font-semibold text-primary-foreground hover:bg-primary/90">Execute Order</Button><p className="text-center text-xs text-muted-foreground">Orders execute during regular market hours.</p></CardContent></Card>
          </aside>
        </div>
      </main>
    </div>
